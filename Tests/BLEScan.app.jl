...
IID_IAsyncOperationCompletedHandler_BluetoothLEDevice = GUID(0x9156b79f, 0xc54a, 0x5277, 0x8f8b, 0xd2cc43c7e004)
...
@interface IBluetoothLEDevice begin
    @inherit IInspectable
    get_DeviceId::Ptr{Cvoid}
    get_Name(this::Ptr{IBluetoothLEDevice}, value::Ptr{HSTRING})::HRESULT
    get_GattServices::Ptr{Cvoid}
    get_ConnectionStatus::Ptr{Cvoid}
    get_BluetoothAddress(this::Ptr{IBluetoothLEDevice}, value::Ptr{UInt64})::HRESULT
    get_GattServices_2::Ptr{Cvoid}
    add_NameChanged::Ptr{Cvoid}
    remove_NameChanged::Ptr{Cvoid}
    add_GattServicesChanged::Ptr{Cvoid}
    remove_GattServicesChanged::Ptr{Cvoid}
    add_ConnectionStatusChanged::Ptr{Cvoid}
    remove_ConnectionStatusChanged::Ptr{Cvoid}
end
...
function AsyncCompletedHandler_QueryInterface(this::Ptr{IAsyncOperationCompletedHandler}, riid::Ptr{GUID}, ppv::Ptr{Ptr{Cvoid}})::HRESULT
    guid = unsafe_load(riid)
    # @info "AsyncCompletedHandler_QueryInterface: $guid"
    if guid == IID_IUnknown || guid == IID_IAsyncOperationCompletedHandler_BluetoothLEDevice
        unsafe_store!(ppv, this)
        return S_OK
    end
    unsafe_store!(ppv, C_NULL)
...
function AsyncOperationCompletedHandler_Invoke(this::Ptr{IAsyncOperationCompletedHandler}, asyncInfo::Ptr{IAsyncOperation}, asyncStatus::AsyncStatus)::HRESULT
    # @info "Async operation completed with status: $asyncStatus $asyncInfo"
    if asyncStatus == Completed
        GetResults(asyncInfo, ppv) |> AssertSuccess
        device = Ptr{IBluetoothLEDevice}(ppv[]) |> Ref
        # @info "Got device: $(device[])"
        if device[] != C_NULL
            hname = HSTRING() |> Ref
            get_Name(device[], hname) |> AssertSuccess
            pname = WindowsGetStringRawBuffer(hname[], C_NULL)
            len = WindowsGetStringLen(hname[])
            name = unsafe_wrap(Array, pname, len) |> v -> transcode(String, v)
            addr = UInt64(0) |> Ref
            get_BluetoothAddress(device[], addr) |> AssertSuccess
            addrstr = @sprintf("%012X", addr[])
            @info "$addrstr: $name"
        end
    else
        @error "Async operation failed with status: $asyncStatus"
    end
    return S_OK
end
