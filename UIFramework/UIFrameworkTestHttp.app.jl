...
function onCreate(host::HTTPHost)
    image_element = MyCustomPixmapElement()
    createHTTPElementHost(host, image_element, IDC_IMAGE, 0, 0, 100, 100)

    text_element = Element(onPaint = element_onPaint, userData = "Simple Text Element")
    createHTTPElementHost(host, text_element, IDC_TEXT, 0, 0, 100, 100)

    ok_button = Button("OK")
    ok_button.onClicked = () -> @info "OK Clicked"
    createHTTPElementHost(host, ok_button, IDC_OK, 0, 0, 100, 100)

    cancel_button = Button("Cancel")
    cancel_button.onClicked = () -> @info "Cancel Clicked"
    createHTTPElementHost(host, cancel_button, IDC_CANCEL, 0, 0, 100, 100)
end

function onResize(host::HTTPHost, w::Int32, h::Int32)
...
function main()
    ...
    host = createHTTPHost(8080, 512, 512)
    onCreate(host)
    layout(host, _layout)
    renderAll(host)

    httpEventLoop(host, onResize)

    destroyHTTPHost(host)
    @info "UI End"
end
